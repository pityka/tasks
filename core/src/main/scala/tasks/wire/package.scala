...
package object wire {
    ...
    implicit val throwableDecoder: Decoder[Throwable] = Decoder.decodeString.emap(str => Either.catchNonFatal(new RuntimeException(str)).leftMap(t => "Throwable"))


    implicit val fileEncoder : Encoder[File] = Encoder.encodeString.contramap[File](_.getAbsolutePath)
    implicit val fileDecoder : Decoder[File] = Decoder.decodeString.emap(str => Either.catchNonFatal(new File(str)).leftMap(t => "File"))
