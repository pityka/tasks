...
trait GroupedOps {
  ...
  def groupedTotal[A: SerDe](taskID: String, taskVersion: Int)(implicit
      r: SerDe[Int],
      rs: SerDe[Seq[A]]
  ): Partial[EColl[A], EColl[Seq[A]]] =
    Partial({ case data1 =>
      resourceRequest =>
        implicit tsc =>
  ...
  def grouped[A: SerDe](
      ...
      r: SerDe[Int],
      rs: SerDe[Seq[A]]
  ): Partial[(EColl[A], Int), EColl[Seq[A]]] =
    Partial({ case (data1, n: Int) =>
      resourceRequest =>
        implicit tsc =>
