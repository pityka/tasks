...
/** High-level, logged wrappers around the smithy4s EC2 client.
  ...
  *   - the compensating "cancel spot then terminate instance if fulfilled"
  *     dance for `shutdownPendingNode`,
  *   - server-side filter usage (never client-side scans),
  *   - scribe logging for every entry / error so failures are traceable.
  */
final class EC2Operations(ec2: EC2[IO]) {
  ...
  def describeInstance(id: String): IO[Option[Instance]] =
    IO(scribe.info(s"describeInstances(id=$id)")) *>
      ec2
        .describeInstances(instanceIds = Some(List(InstanceId(id))))
        .map { r =>
          r.reservations
            .getOrElse(Nil)
  ...
  /** Cancel a spot request, and if it has already spawned an instance,
    * also terminate the instance. This avoids the well-known EC2 pitfall
    * where cancelling a fulfilled spot request leaves the running instance
    * behind (and billing continues).
    */
  def cancelSpotAndTerminate(spotRequestId: String): IO[Unit] = {
    val sri = SpotInstanceRequestId(spotRequestId)
    for {
      _ <- IO(scribe.info(s"cancelSpotInstanceRequests($spotRequestId)"))
      _ <- ec2
        .cancelSpotInstanceRequests(spotInstanceRequestIds = List(sri))
        .void
        .onError { case e =>
          IO(
            scribe
      ...
      req <- describeSpotRequestById(spotRequestId)
      _ <- req.flatMap(_.instanceId).map(InstanceId.value) match {
        case Some(iid) =>
          IO(
            scribe.info(
              s"spot $spotRequestId already spawned $iid — terminating"
            )
  ...
  private def describeSpotRequestById(
